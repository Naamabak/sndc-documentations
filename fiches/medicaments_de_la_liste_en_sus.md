...
Les médicaments peuvent être délivrés en ville ou en établissement. Les médicaments délivrés en ville sont recueillis dans le DCIR via la table [ER_PHA_R](../tables/DCIR/ER_PHA_F.md) et les codes [CIP](../fiches/medicament.md).
...
::: warning
Les médicaments délivrés par les établissements peuvent être déconditionnés et délivrés par fraction. Attention les variables numériques (`UCD_FRC_COE` et `COE_TAU`) indiquant le fractionnement en pourcentage dans les tables des médicaments du PMSI et la table [ER_UCD_R](../tables/DCIR/ER_UCD_F.md) ne sont pas toujours sur la même échelle.
:::
